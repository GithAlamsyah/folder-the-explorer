 ...
 */
export const folderController = (adapter: IDatabaseAdapter) => {
    const folderRepository = adapter.createRepository();
    ...
    return new Elysia({ prefix: '/api/v1/folders' })
        ...
        .get('/:id/children', async ({ params, set }) => {
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : 'Failed to retrieve folder children';
                
                if (errorMessage === 'Parent folder not found') {
                    set.status = 404;
                } else {
                    set.status = 500;
                }
